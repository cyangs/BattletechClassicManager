...
class FireCalculations:
    ...
    def resolve(self) -> WeaponShot:
        ...
        if self.weapon.cluster:
            ...
            cluster_hits = ClusterHitsTable.get_hits(max_cluster_size=self.weapon.num_shots,
                                                     roll=cluster_hit_roll)

            cluster_groups = self.weapon.cluster_damage or 1
            full_groups, remainder = divmod(cluster_hits, cluster_groups)
            groups = [cluster_groups] * full_groups
            if remainder:
                groups.append(remainder)

            """ Iterate through the groupings and roll location"""
            group_hits: list[ClusterHit] = []
            for cluster_dmg_group in groups:
                ...
                group_hits.append(ClusterHit(location=cluster_hit_location, damage=cluster_dmg_group))
